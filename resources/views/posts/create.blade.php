...
<form action="/posts" method="post" enctype="multipart/form-data" x-on:submit="postSubmit($refs.body)" class="post-form my-4 p-4 grid grid-cols-1 md:grid-cols-3 gap-4">
    ...
    <main class="md:col-span-2 space-y-4">
        ...
        <div class="bg-white rounded border">
            @error('body')
            <small class="block m-4 text-danger">{{ $message }}</small>
            @enderror
            <textarea name="body" id="editor" class="text-base" x-init="CKEDITOR.replace('editor')" x-cloak>{{ old('body') }}</textarea>
        </div>
    </main>
    <!-- Right side -->
    <div class="bg-white p-4 rounded border h-fit sticky top-4 space-y-4">
        <div class="">
            ...
            <img x-ref="previewContainer" src="" id="preview" class="aspect-auto bg-black/5 border-2 border-black/20 border-dashed text-black/60 my-2 min-h-[150px] relative after:content-['preview'] after:inline-block after:absolute after:bottom-1/2 after:right-1/2 after:translate-x-1/2 after:translate-y-1/2">
